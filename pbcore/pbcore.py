...
class PBCoreDocument:
	...
	def add_instantiation(self, pbcoreInstantiationPath):
		...
		instantiation = self.add_SubElement(self.descriptionRoot,'pbcoreInstantiation',nsmap=self.NS_MAP)
		# print(self.pbcoreInstantiation.xpath('/p:pbcoreInstantiationDocument/*',namespaces=self.XPATH_NS_MAP))
		for element in pbcoreInstantiation.xpath('/p:pbcoreInstantiationDocument/*',namespaces=self.XPATH_NS_MAP):
			# print(element.tag)
		...
		for field in pbcore_map.BAMPFA_FIELDS:
			...
			if field in descMetadataFields:
				...
				if mappedPbcore["TEXT"] == "value":
					...
					'''
					if level == "WORK":
						self.add_SubElement(self.descriptionRoot,mappingTarget,attrib=mappingAttribs,_text=mdValue,nsmap=self.NS_MAP)
					else:
						# if it applies to an instantiation, 
						...
						xpathExpression = "/pbcoreDescriptionDocument/pbcoreInstantiation[contains(./p:instantiationIdentifier,'{}')]".format(assetBasename)
						# print(xpathExpression)
						targetInstantiation = self.descriptionRoot.xpath(xpathExpression,namespaces=self.XPATH_NS_MAP)
						# print(targetInstantiation)
						if not targetInstantiation == []:
							self.add_SubElement(targetInstantiation[0],mappingTarget,attrib=mappingAttribs,_text=mdValue,nsmap=self.NS_MAP)
					if "SIBLING_FIELDS" in mappedPbcore:
						...
						for key, value in mappedPbcore["SIBLING_FIELDS"].items():
							...
							if "ATTRIBUTES" in subSib:
								...
								attrib = {}
							text = subSib["TEXT"]
							self.add_SubElement(self.descriptionRoot,key,attrib=attrib,_text=text,nsmap=self.NS_MAP)
				else:
					'''
					...
					'''
					if level == "WORK":
						top = self.add_SubElement(self.descriptionRoot,mappingTarget,attrib=mappingAttribs,nsmap=self.NS_MAP)
						print(top)
						for key,value in mappedPbcore["SUBELEMENTS"].items():
							...
							if "ATTRIBUTES" in mappedPbcore["SUBELEMENTS"][key]:
								...
								attrib = {}
							subelement = self.add_SubElement(top,key,attrib=attrib,nsmap=self.NS_MAP)
							print(subelement)
