# ingestfiles
This is an alpha release webapp for use in BAMPFA a/v digital preservation. It uses the Flask framework in a pretty basic way to 
...
The pymm-generated SIP consists of a predefined folder structure containing the master file, its derivatives, and technical metadata about each file, including checksums. `pymm` creates a derivative access copy that is appropriate for use in ResourceSpace and for research access. `pymm` copies the MySQL database structure used in `mm` to record [PREMIS](https://en.wikipedia.org/wiki/Preservation_Metadata:_Implementation_Strategies) events and file characteristics.
...
* Currently LTO write is done in a manual selection process. Sounds like we want this run as a (weekly?)  `cron` job, but the manual process provides details like total AIP size and available space left on a tape.

## Setup

We are running Flask in production Apache using the `mod_wsgi` module (see also the [Flask](flask-ui-notes) section). Honestly, this sucks to set up.

`pymm` requires some configuration of input and output paths, database configuration (only need to do this once at setup). There are a couple of non standard Python 3 libraries used (`lxml` and `Levenshtein`).
...
The index.schema files are also parsed the contents are indexed in the `pymm` database. [UNDER REVISION - the parsing module is ready but I need to insert the actual call during the unmount call]
...
* Metadata Schemas for non-film-collection resources.
  * Add a form per item to ingest that will create a CSV /JSON of descriptive metadata.
